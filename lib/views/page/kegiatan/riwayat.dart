...
import 'package:eahmindonesia/views/page/meeting/tambah.dart';
...
class RiwayatKegiatanPage extends StatefulWidget {
  final String? token;
  final String? userid;
  const RiwayatKegiatanPage({super.key, this.token, this.userid});

  @override
  State<RiwayatKegiatanPage> createState() => _RiwayatKegiatanPageState();
}

class _RiwayatKegiatanPageState extends State<RiwayatKegiatanPage> {
  ...
  void _dataPesanan(token, userid) async {
    KegiatanModel? data = await kegiatanController.getkegiatan(token, userid, 'history');
    if (mounted) {
      setState(() {
        _kegiatanModel = data;
      });
