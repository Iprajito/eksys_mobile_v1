...
class TambahKegiatanPage extends StatefulWidget {
  final String? token;
  final String? userid;
  const TambahKegiatanPage({super.key, this.token, this.userid});
...
class _TambahKegiatanPageState extends State<TambahKegiatanPage> {
  ...
  TempKegiatanDispacthModel? _tempKegiatanModel;
  ...
  TextEditingController aktifitasController = TextEditingController();
  ...
  void initState() {
    ...
    _dataTempKegiatan(widget.token.toString(), widget.userid.toString());
  }
  ...
  void _dataTempKegiatan(String token, String userid) async {
    kegiatanController = KegiatanController();
    TempKegiatanDispacthModel? dataTempKegiatan = await kegiatanController.gettempkegiatandispatch(token, userid);
    setState(() {
      _tempKegiatanModel = dataTempKegiatan;
    });
  }

  void _saveKegiatan() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (tanggal.isEmpty) {
      ...
      } else if (namakegiatan == null || namakegiatan!.isEmpty) {
        shoMyBadDialog(
          context: context,
          title: "Tambah Kegiatan",
      ...
      } else if (_tempKegiatanModel == null || _tempKegiatanModel!.data.isEmpty) {
        shoMyBadDialog(
          context: context,
          title: "Tambah Kegiatan",
        ...
        kegiatanController = KegiatanController();
        var response = await kegiatanController.saveKegiatan(
  ...
  void _delTempKegiatanDetail() async {
    showLoadingDialog(context: context);
    kegiatanController = KegiatanController();
    final NavigatorState navigator = Navigator.of(context);
    await kegiatanController.deltempkegiatandispatch(widget.token.toString(), widget.userid.toString(), '');
    hideLoadingDialog(context);
    navigator.pop();
  }

  _delTempKegiatanDetailById(id) async {
    showLoadingDialog(context: context);
    kegiatanController = KegiatanController();
    var response = await kegiatanController.deltempkegiatandispatch(
  ...
  Future<void> navigateToTambahKegiatanPage() async {
                        ...
                        BootstrapRow(
                          ...
                          children: [
                            BootstrapCol(
                              ...
                              child: TextFormField(
                                // maxLines: 3,
                                controller: aktifitasController,
                                cursorColor: Colors.grey,
                                decoration: InputDecoration(
                                    labelText: 'Aktifitas',
                                    labelStyle:
                                ...
                                children: [
                                  Text("Data Undangan",
                                      style: TextStyle(
                                          ...
                                          fontSize: 14)),
                                  GestureDetector(
                                    onTap: () => navigateToTambahKegiatanPage(),
                                    child: Row(
                                      children: [
                                                  ...
                                                  itemBuilder:
                                                    ...
                                                    var anggota =
                                                        _tempKegiatanModel!
                                                            ...
                                                            .anggota
                                                            .toString();
                                                            var nama_wilayah = _tempKegiatanModel!.data[index].nama_wilayah.toString();
                                                            var totanggota = _tempKegiatanModel!.data[index].totanggota.toString();
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () => _saveKegiatan(), //_savePesanan,
  ...
  void _saveProduks() async {
    ...
    if (_entryManager.entries.length > 0) {
      ...
      for (var entry in _entryManager.entries) {
        await kegiatanController.saveTempKegiatanDispatch(
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
          appBar: AppBar(
            ...
            title: const Text("Undangan", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
            backgroundColor: const Color.fromARGB(255, 0, 48, 47),
            bottom: const TabBar(
